...
export const modules: ModuleSpec[] = [
  ...
  },
  {
    ...
    sport: 'baseball',
    status: 'wip',
    summary: '把 BPE 揮棒結果的擊球點（球棒碰到球的位置）以捕手視角畫在好球帶九宮格上，看偏左右多少、多高。好球帶由打者級別推算；視野固定 150 × 200 cm，不同事件的點可以直接比位置，超出視野的點照座標擴大顯示、不裁切。',
    ...
    tech: [
      ...
      '不依「在不在好球帶內」換色——擊球點不是好壞球判定',
      '元件只吃 x／z 數字、不依賴 BPE 格式；檢查關卡與缺值規則在共用資料層 bpe-data',
      'Vue 3 / Nuxt 4',
    ],
    data: {
      summary: '演算法端 BPE 揮棒結果（result-envelope-v2）的 contact.point_cm（cm）與擊球點左右、高度兩項數值。23 筆 2026-05-12 龍潭實測，其中 14 筆有擊球點。好球帶不在資料裡，由打者級別推算。',
  ...
  },
  {
    ...
    sport: 'baseball',
    status: 'wip',
    summary: '把 BPE 揮棒結果的預測落點畫在俯視球場圖上，看球預測會落在哪、飛多遠、偏左偏右。球場邊界照規範公式畫（邊線 102 m、中外野 122 m）；落點在界外或本壘後方照樣畫、不判斷，超出視野時自動擴大。',
    ...
    tech: [
      ...
      '固定視野 x ±80、y −10–128 m；落點超出才擴大（23 筆只有事件 #11 會觸發），不 clamp',
    ...
    handoff: {
      ...
      dependencies: ['Nuxt 4（showcase 靠 auto-import 取得 ref/computed，模組本身只 import vue）', 'tailwindcss（線條與文字的顏色 class）'],
      flexPoints: [
        '落點旁第二行文字 label（showcase 放預測飛行距離）',
