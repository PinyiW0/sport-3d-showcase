...
核對交接報告的逐筆數字（四類分布 14／1／2／6、有樣本無擊球點卻有落點、球體幀數等），樣本不在時自動跳過。
...
- 缺 `time_s` 的幀放不上時間軸，整幀略過。代價是之後的幀索引會往前移一格，拖尾「往前數 19 幀」的時間窗
  會因此多涵蓋一幀。23 筆樣本每幀都有 `time_s`，目前不會發生；規範也沒定義這種情況
...
- 兩種寫法（`core/format.ts`）：
  - `formatMetric`：**原樣顯示、不四捨五入**。小數位數不固定（1～3 位），且 23 筆樣本數值皆未經人工確認，
    要看精確值時一律用它
...
- 擊球方向另用 `formatExitDirectionBrief`：講偏哪一邊、角度取絕對值（2.9 → 「一壘側 2.9°」，−35.64 → 「三壘側 35.6°」，
  四捨五入後是 0 寫「正中 0.0°」）。正負代表哪一邊是推定，見下方「規範未涵蓋的情況」第 3 點
...
2. 有擊球點的 14 筆 `contact.time_s` 與 `trigger_time_s` 完全相等；無擊球點時 `trigger_time_s` 仍有值
3. `exit_direction` 的正負代表哪一邊沒有定義。畫面寫成「正值＝一壘側」是從樣本推定：有落點的 17 筆裡 16 筆與落點 x 同號，
   其中一筆例外、落點幾乎在正中。方向角也不等於本壘到落點的方位角

異常值一律照座標畫、不做過濾，各模組 README 的「已知限制」另有說明。
