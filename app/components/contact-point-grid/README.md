...
原版預設固定 **x ∈ [−75, 75]、z ∈ [0, 200]（cm）**。23 筆樣本的擊球點都在這個範圍內，全都裝得下且不會觸發擴大——固定視野讓不同事件的擊球點可以直接比位置，不會因為單一事件的落點而跳動縮放。

**簡約版**的視野換一套：x 取「好球帶左右各外推 25 cm」與「±48 cm」的較寬者（本壘板寬固定，所以恆為 ±48），z 從 −30 cm（留給本壘板與打擊區示意）到好球帶上緣 +15 cm，上緣隨打者級別變。視野小很多：成棒時 23 筆裡有幾筆事件的擊球點落在視野外，切到這幾筆會擴大視野並顯示「已自動擴大」標籤。
...
- 擊球點高度異常值：有幾筆樣本的 `contact_height` 高於一般好球帶上緣（成棒約 92 cm）。規範要求「照座標畫」，畫面上不做任何過濾或修正，數值是否合理規範沒有說明。
- `contact_side`／`contact_height` 的定義暫定與 Trackman 的 `ContactPosition` 相同，這兩個欄位的量測基準規範未定義。
- 23 筆樣本的數值皆未經人工確認；9 筆事件的 `contact` 為 `null`（無擊球點），依規範不畫。
