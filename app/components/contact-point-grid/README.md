...
| 檔案 | 責任 |
|------|------|
| `ContactPointGrid.vue` | SVG 渲染：地面線、本壘板正視窄帶、刻度、好球帶九宮格與格號、擊球點與輔助線、其他事件的灰點 |
...
| `app/components/modules/ContactPointGridShowcase.vue` | 消費端 showcase：接 BPE 樣本、事件選單與上一筆／下一筆、打者級別選單、輔助線與淺色畫布開關、畫布標籤與圖例、數值面板（可用 NuxtUI，不隨模組搬走） |
...
擊球點落在預設視野外時（目前樣本沒有，但未來資料可能有），依規範「照座標畫，不裁切」（`baseball-field-coordinates.md` §9「不要只做 clamp」）：把視野擴大到剛好包住該點，再留 15 cm 邊。`core/contactGridScale.ts` 回傳的 `expanded` 旗標讓外面知道視野已經改變；showcase 用它在畫布上方加一個「已自動擴大」標籤。
