...
> **下列 grep pattern 是常見命名訊號，非窮舉白名單**：變數 / 狀態名不在清單、但語意確實是 loading / empty / toggle 等（中文或領域命名如 `載入中`、`rows`、`records`、`抽屜開啟`）時，**以語意歸類，別因名字不在表上就降級 visual** 而漏補測試。拿不準時看語意、必要時問操作者。
